...
public class SecurityConfig {
    ...
    public SecurityFilterChain filterChain(HttpSecurity http, Jwt2AuthenticationConverter authenticationConverter) throws Exception {
        ...
        http.cors(customizer -> {
            ...
            corsConfiguration.setAllowedOrigins(Arrays.asList("http://localhost:8000")); // REACT ZEZWOLENIE
            corsConfiguration.setAllowedMethods(Arrays.asList("GET", "POST", "PUT", "DELETE"));
            corsConfiguration.setAllowedHeaders(Arrays.asList("*"));
            corsConfiguration.setAllowCredentials(true);

            customizer.configurationSource(corsConfigurationSource(corsConfiguration));
        });
    ...
    private CorsConfigurationSource corsConfigurationSource(CorsConfiguration config) {
